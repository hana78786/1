...
public class Topoki_Client {
	...
	public static void main(String[] args) {
		...
		boolean topoki = false;
		boolean noMoney = false;
		Player player = new Player();
		Nembi_Store ns = new Nembi_Store();
		int answer = 0;
		...
		종료: while (true) {
			if (player.getMoney() <= 0 && (ns.getTock() == 0 || ns.getDangmen() == 0 || ns.getDashida() == 0
					|| ns.getEgg() == 0 || ns.getOdeng() == 0 || ns.getGochujang() == 0)) {
				try {
					Thread.sleep(200);
					System.out.println("=========");
			...
			try {
				...
				case 1:
						...
						if (Ox.equalsIgnoreCase("o")) {
							...
							if (noMoney == false) {// 참가금이 충분할때
								...
								if (answer == 3) {
									...
									player.setMoney(player.getMoney() + gameMoney * 2);// 참가비의 2배추가
									answer = 0;// 정답갯수 초기화
									game = true;
									break;
								} else {
									System.out.println("아쉽네요.. 참가금은 돌려드릴수 없습니다");
									player.setMoney(player.getMoney() - gameMoney); // 참가비 만큼 감소
									answer = 0;// 정답갯수 초기화
									game = true;
									break;
								}
					...
					break;
				case 2:
					System.out.println("소지금 입니다 : " + player.getMoney());
					break;
				case 3:
					// 냄비 메뉴
