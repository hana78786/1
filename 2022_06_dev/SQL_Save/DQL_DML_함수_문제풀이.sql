...
select emp_name, floor((sysdate-hire_date)/1000)*0.1*salary "특별보너스", salary, floor((sysdate-hire_date)/1000), hire_date
...
select emp_name"이름", extract(year from to_date(substr(emp_no,1,6)))"년생" , nvl(bonus,0)"보너스" from employee
...
--	   인원
--	ex) 3명
select count(*)||'명' "인원" from employee
...
select emp_name, job_code, to_char(salary*12+salary*nvl(bonus,0),'l999,999,999')"연봉" from employee;
...
select emp_name, dept_code, to_char(to_date(substr(emp_no,1,6)),'YYYY"년 "MM"월 "DD"일"') from employee
