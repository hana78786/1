...
select emp_name ,dept_code, salary from employee a
where dept_code = (select dept_id from department where dept_title ='기술지원부') and
...
--사번,이름,매니저 이름,월급(만원단위부터)을 구하시오
 ...
 select a.emp_id, a.emp_name, b.emp_name, salary/10000||'만원'"월급" from employee a
...
select (select dept_title from department where a.dept_code = dept_id)"부서", 
floor(avg(salary))"평균" from employee a
...
select emp_name, dept_code, salary from employee a
where dept_code = (select dept_id from department where dept_title = '기술지원부')
and not exists (select 1 from employee where salary>a.salary and dept_code = (select dept_id from department where dept_title = '기술지원부'));
...
--사번,이름,매니저 이름,월급(만원단위부터)을 구하시오
 ...
 select a.emp_id, a.emp_name, b.emp_name, salary/10000||'만원'"급여" from employee a
 join (select emp_id, emp_name from employee) b
 on a.manager_id = b.emp_id
 where salary>
 (select avg(salary) from employee);
...
select (select dept_title from department where dept_id = a.dept_code)"부서명", 
